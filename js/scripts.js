...
/* Inica script para hacer cambio de imagenes Hover del menu inferior Footer */

document.addEventListener('DOMContentLoaded', function() {
  const icons = document.querySelectorAll('.icoConcierge, .icoDescubre, .icoPerfilado');

  const hoverImages = {
      'icoConcierge': {
          original: 'img/menu/aiconcierge.svg',
          hover: 'img/menu/aiconciergeHover.svg'
      },
      'icoDescubre': {
          original: 'img/menu/aisearch.svg',
          hover: 'img/menu/aisearchHover.svg'
      },
      'icoPerfilado': {
          original: 'img/menu/aiperfilado.svg',
          hover: 'img/menu/aiperfiladoHover.svg'
      }
  };

  // Función para actualizar la imagen según la clase .menuIconActived
  function updateIconImage(icon) {
    const img = icon.querySelector('img');

    if (icon.classList.contains('menuIconActived')) {
      // Si tiene .menuIconActived, muestra la imagen hover
      if (icon.classList.contains('icoConcierge')) {
        img.src = hoverImages.icoConcierge.hover;
      } else if (icon.classList.contains('icoDescubre')) {
        img.src = hoverImages.icoDescubre.hover;
      } else if (icon.classList.contains('icoPerfilado')) {
        img.src = hoverImages.icoPerfilado.hover;
      }
    } else {
      // Si no tiene .menuIconActived, muestra la imagen original
      if (icon.classList.contains('icoConcierge')) {
        img.src = hoverImages.icoConcierge.original;
      } else if (icon.classList.contains('icoDescubre')) {
        img.src = hoverImages.icoDescubre.original;
      } else if (icon.classList.contains('icoPerfilado')) {
        img.src = hoverImages.icoPerfilado.original;
      }
    }
  }

  // Actualizamos las imágenes inmediatamente al cargar la página
  icons.forEach(icon => {
    updateIconImage(icon); // Se ejecuta para cada ícono al cargar la página

    // Evento mouseenter (cuando el ratón entra al icono)
    icon.addEventListener('mouseenter', function() {
      const img = icon.querySelector('img');

      // Si no tiene .menuIconActived, cambiar a la imagen hover
      if (!icon.classList.contains('menuIconActived')) {
        if (icon.classList.contains('icoConcierge')) {
          img.src = hoverImages.icoConcierge.hover;
        } else if (icon.classList.contains('icoDescubre')) {
    ...
    // Evento mouseleave (cuando el ratón sale del icono)
    icon.addEventListener('mouseleave', function() {
      const img = icon.querySelector('img');

      // Si no tiene .menuIconActived, volver a la imagen original
      if (!icon.classList.contains('menuIconActived')) {
        if (icon.classList.contains('icoConcierge')) {
          img.src = hoverImages.icoConcierge.original;
        } else if (icon.classList.contains('icoDescubre')) {
